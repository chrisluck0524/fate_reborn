...
export const FATE_LAYOUT_STYLE = `
  ...
  .fate-standalone #arena.fate-table-layout > #control > .control.fate-skill-control > div:last-child {
    display: none;
  }
...
function classifyControls() {
  ...
  for (const node of Array.from(ui.control.children)) {
    node.classList.remove("fate-skill-control", "fate-confirm-control", "fate-end-control");
    if (node === ui.skills || node === ui.skills2 || node === ui.skills3) {
