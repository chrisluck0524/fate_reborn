...
export const FATE_LAYOUT_STYLE = `
  ...
  .fate-standalone #arena.fate-table-layout > .fate-table-ui {
    position: absolute;
    inset: 0;
    /* Keep the custom panels above the native player nodes.  The table layer
       itself does not catch events; only panels that opt in below do. */
    z-index: 10;
    pointer-events: none;
  }
  ...
  .fate-standalone .fate-persistent-skill[aria-disabled="true"] {
    pointer-events: none;
  }
  ...
  /* In the desktop engine handcards1 is normally a direct child of #arena
     (the #me descendant form is used by a few layouts).  Give the root form
     the same central hand strip geometry before applying flex-end alignment;
     otherwise its 100% width makes right-aligned cards sit under the skill
     panel and the self hero. */
  .fate-standalone #arena.fate-table-layout > #handcards1 {
    left: 13.4% !important;
    top: auto !important;
    right: auto !important;
  ...
  /* Each opponent keeps its native marks node inside an explicit, narrow
     status column.  Anchoring the column to the player avoids the native
     marks positioning rules leaking into the Fate layout. */
  .fate-standalone #arena.fate-table-layout > .player > .fate-opponent-status {
    box-sizing: border-box;
    position: absolute !important;
    top: 4px !important;
  ...
  .fate-standalone #arena.fate-table-layout > .player.fate-status-right > .fate-opponent-status {
    left: calc(100% + 4px) !important;
  }

  .fate-standalone #arena.fate-table-layout > .player.fate-status-left > .fate-opponent-status {
    right: calc(100% + 4px) !important;
  }

  .fate-standalone #arena.fate-table-layout > .player > .fate-opponent-status > .marks {
    box-sizing: border-box;
  ...
  .fate-standalone #arena.fate-table-layout > .player > .fate-opponent-status > .marks > div {
    position: relative !important;
    left: 0 !important;
    top: 0 !important;
  ...
  .fate-standalone #arena.fate-table-layout > .player > .fate-opponent-status > .marks > div:first-child {
    display: none !important;
  }

  .fate-standalone #arena.fate-table-layout > .player > .fate-opponent-status > .marks > div > .markcount {
    left: 15px !important;
    top: 13px !important;
  ...
  /* Keep a fallback for a brief initialization frame before the status
     wrapper is attached. */
...
function statusSideForSlot(slot) {
  // Put the marker column between the opponent and the playfield.  The top
  // row uses the same inner-facing rule: top-right faces left, while the
  // other top seats face right so their columns do not sit outside the table.
  if (slot.includes("right") || slot === "fate-seat-top-right") return "left";
  return "right";
}

function ensureOpponentStatus(player) {
  if (!player || player === game.me) return null;
  const marks = player.node?.marks;
  if (!marks) return null;
  let status = Array.from(player.children).find(child => child.classList?.contains("fate-opponent-status"));
  if (!status) {
    status = document.createElement("div");
    status.className = "fate-opponent-status";
    player.appendChild(status);
  }
  if (marks.parentNode !== status) status.appendChild(marks);
  return status;
}
...
function updateSeatLayout() {
  ...
  for (const player of game.players) {
    if (player !== game.me) player.classList.remove(...SEAT_SLOTS, "fate-marks-left", "fate-marks-right", "fate-status-left", "fate-status-right");
  }
  ...
  opponents.forEach((player, index) => {
    ...
    player.classList.add(`fate-status-${statusSideForSlot(slot)}`);
    ensureOpponentStatus(player);
  });
...
function createPersistentSkills(panel) {
  ...
  for (const skill of getPermanentSkills()) {
    ...
    button.addEventListener("click", () => {
      if (!button.classList.contains("fate-skill-ready")) return;
      const native = findNativeSkillButton(panel, skill);
      if (!native) return;
      // Calling the engine handler directly keeps the original skill event,
      // backup/restore flow and game.resume semantics intact even though the
      // native control is visually hidden in the custom panel.
      if (typeof ui.click?.control === "function") ui.click.control.call(native);
      else native.click();
    });
...
function findNativeSkillButton(panel, skill) {
  const controls = [];
  for (const control of [ui.skills, ui.skills2, ui.skills3]) {
    if (control && !controls.includes(control)) controls.push(control);
  }
  if (panel) {
    for (const control of panel.querySelectorAll(".control.fate-skill-control")) {
      if (!controls.includes(control)) controls.push(control);
    }
  }
  for (const control of controls) {
    for (const child of Array.from(control.children || [])) {
      if (child.link === skill || child.dataset?.fateSkill === skill) return child;
    }
  }
  return null;
}
...
function syncPersistentSkills(panel) {
  ...
  const nativeButtons = controls.flatMap(control => Array.from(control.children || []));
