...
test('冰封在移动前标记牌，目标标记保持到施放者回合结束，结束时返还原牌并清理状态',async()=>{
  const cards=[{},{}], tags=new Set(),skills=new Set();let gained;
  const target={playerid:'target',isAlive:()=>true,addSkill(skill){skills.add(`target:${skill}`);},removeSkill(skill){skills.delete(`target:${skill}`);},async gain(value){gained=value;}};
