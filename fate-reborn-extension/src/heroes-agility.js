...
async function razorField(player) {
  ...
  if (!paid.bool) return;
  const targets = await player.chooseTarget([1, paid.cards.length], "等离子场：选择攻击范围内的目标", (card,source,target)=>source!==target && source.inRange(target)).forResult();
...
async function nyxBurn(player) {
  ...
  if (!target) return;
  await player.useCard({ name: "fate_nyx_burn_effect", isCard: true }, target);
}
...
export const AGILITY_HERO_CARDS = {
  fate_medusa_snake_effect: trick(() => true, async (event, trigger, player) => {
    ...
    for (const target of actionOrder(player,game.players).filter(target => target !== player)) {
    ...
    const recipients=new Set(); for (const card of pool) { const target = (await player.chooseTarget("秘术异蛇：选择尚未分配的角色（每人一张）", true, (card,source,target)=>!recipients.has(target)).forResult()).targets?.[0]; if (target) {recipients.add(target); await target.gain(card, "gain2");} }
  }),
  fate_razor_field_effect: trick(() => true, async (event, trigger, player) => {
    const suits = event.card.storage?.suits || [];
    for (const target of orderedTargets(player,game.players,game.players.filter(target=>(event.card.storage?.targetIds || []).includes(target.playerid)))) {
  ...
  fate_nyx_burn_effect: trick(() => true, async (event, trigger, player) => {
    const target=event.target, discarded=[];
    if(target.countCards("h")) { const result=await player.discardPlayerCard(target,"h",true).forResult(); discarded.push(...(result.cards||[])); }
    if(target.countCards("h")) {
      const guess=await player.chooseControl("红色","黑色").set("prompt","法力燃烧：猜测另一张手牌颜色").forResult();
      const card=target.getCards("h").randomGet(); await target.showCards(card,"法力燃烧");
      if((guess.control==="红色"?"red":"black")===get.color(card)) {await target.discard(card);discarded.push(card);}
    }
    const available=discarded.filter(card=>get.position(card,true)==="d");
    if(available.length && player.countCards("h")) {
      const paid=await player.chooseToDiscard("h",1).set("prompt","法力燃烧：可弃1张手牌，将目标弃掉的一张牌交给任意角色").forResult();
      if(paid.bool) {
        const selected=available.length===1?available[0]:(await player.chooseButton(["法力燃烧：选择交出的牌",available],true).forResult()).links?.[0];
        const recipient=(await player.chooseTarget("法力燃烧：选择获得牌的角色",true).forResult()).targets?.[0];
        if(selected && recipient && get.position(selected,true)==="d") await recipient.gain(selected,"gain2");
      }
    }
  }),
...
export const AGILITY_HERO_SKILLS = {
  ...
  fate_juggernaut_omnislash: { trigger: { player: "phaseDrawBegin2" }, direct: true, filter(event, player) { return event.num > 0 && true; }, async content(event, trigger, player) { if ((await player.chooseBool("无敌斩：放弃摸牌，本回合黑色手牌均可当攻击且攻击次数不限？").forResult()).bool) { trigger.cancel(); player.addTempSkill("fate_juggernaut_omnislash_effect", "phaseAfter"); } } },
  ...
  fate_vengeful_swap: { trigger: { global: "fateJudgeBeforeReveal" }, direct: true, filter(event,player) { return !event.directresult && player.countCards("h")>0; }, async content(event,trigger,player) { const result=await player.chooseCard("h","移形换位：选择1张手牌作为尚未翻开的判定牌").forResult(); if(result.cards?.[0]) {player.logSkill("fate_vengeful_swap");trigger.directresult=result.cards[0];} } },
  fate_vengeful_wave: { charlotte: true, popup: false },
  fate_bloodseeker_rupture: { trigger: { global: "fateCastPhase" }, forced: true, filter(event, player) { return player.countCards("h", card => get.color(card) === "red") > 0; }, async content(event, trigger, player) { const useful = target => target !== player && isUsefulCastTarget("bloodseeker", { attitude: get.attitude(player, target), hand: target.countCards("h") }); if (player !== game.me && !game.hasPlayer(useful)) return; if (player === game.me && !(await player.chooseBool("血之狂暴：是否发动？").forResult()).bool) return; const target = (await player.chooseTarget("血之狂暴：选择一名角色", true).set("ai", target => useful(target) ? hostileTarget(target, player) : -Infinity).forResult()).targets?.[0]; if (!target) return; const paid = await player.chooseToDiscard("h", 1, true).set("filterCard", card => get.color(card) === "red").forResult(); if (paid.bool) await player.useCard({ name: "fate_bloodseeker_rupture_card", isCard: true }, target); } },
  fate_bloodseeker_rupture_effect: { charlotte: true,
    init(player,skill) {player.addSkillBlocker(skill);}, onremove(player,skill) {player.removeSkillBlocker(skill);},
    skillBlocker(skill,player) {if(_status.currentPhase!==player || !_status.event?.getParent?.("phaseUse")?.name?.includes("phaseUse")) return false; return ["fate_death_coil","fate_sacrifice","fate_battle_hunger","fate_jakiro_ice","fate_lich_feast","fate_dazzle_wave","fate_keeper_wave","fate_techies_bomb","fate_medusa_snake","fate_razor_field","fate_nyx_burn"].includes(skill);}, mark: true, marktext: "狂", intro: { content: "本回合出牌阶段只能使用基本牌与被动技能；你造成的攻击伤害+1。" }, mod: { cardEnabled(card,player) { if (_status.currentPhase===player && _status.event?.getParent?.("phaseUse")?.name==="phaseUse" && get.type(card) !== "basic") return false; }, }, trigger: { source: "damageBegin1" }, forced: true, filter(event) { return event.card?.name === "sha"; }, async content(event, trigger) { trigger.num += 1; } },
  ...
  fate_sniper_headshot: { trigger: { player: "useCardToPlayered" }, direct: true, filter(event) { return event.card?.name === "sha"; }, async content(event, trigger, player) { if(!(await player.chooseBool("爆头：是否进行判定？").forResult()).bool) return; player.logSkill("fate_sniper_headshot"); const judge = await player.judge(card => get.color(card) === "black" ? 1 : -1).forResult(); if (judge.bool) {const map=trigger.getParent().customArgs;map[trigger.target.playerid] ||= {};map[trigger.target.playerid].extraDamage=(map[trigger.target.playerid].extraDamage||0)+1;} } },
  ...
  fate_antimage_blink: { enable: ["chooseToUse","chooseToRespond"], hiddenCard(player,name) { return name === "shan" && player.countCards("h", card => get.color(card) === "black") > 0; }, viewAsFilter(player) { return player.countCards("h", card => get.color(card) === "black") > 0; }, filterCard(card) { return get.color(card) === "black"; }, position: "h", viewAs: { name: "shan" }, prompt: "将黑色手牌当闪避使用或打出", ai: { respondShan: true, skillTagFilter(player) { return player.countCards("h", card => get.color(card) === "black") > 0; } } },
  fate_chen_judgement: { trigger: { global: "judgeFixing" }, direct: true, filter(event,player) { return !event.fateRejudged && !event.fateRejudgeChild && player.countCards("h")>0; }, async content(event,trigger,player) {
    if(!(await player.chooseBool("神判：弃1张手牌，令此判定重新进行一次？").forResult()).bool) return;
    const paid=await player.chooseToDiscard("h",1,true).forResult(); if(!paid.bool) return;
    player.logSkill("fate_chen_judgement");trigger.fateRejudged=true;
    const next=trigger.player.judge(trigger.judge);next.fateRejudgeChild=true;
    const result=await next.forResult(); trigger.result={...result};
  } },
...
export const AGILITY_HERO_TRANSLATIONS = Object.fromEntries(Object.entries({
  fate_medusa_snake:["秘术异蛇","出牌阶段限一次：弃1张手牌，逆时针抽取其他角色各1张手牌，连同费用牌每人一张分给不同角色；无手牌者失怒气或受伤害。"], fate_medusa_shield:["魔法护盾","受到伤害前可消耗1怒气并弃1手牌防止伤害。"],
  fate_razor_field:["等离子场","出牌阶段限一次：弃不同花色手牌，令攻击范围内至多等量角色弃未曾弃过花色的牌或受伤害。"], fate_razor_current:["不定电流","受到攻击伤害后可获得该攻击牌。"],
  ...
  fate_bloodseeker_rupture:["血之狂暴","施法阶段可弃红色手牌，令目标本回合出牌阶段只能使用基本牌与被动技能，攻击伤害+1。"], fate_bloodseeker_rupture_effect:["血之狂暴","本回合出牌阶段只能使用基本牌与被动技能；你造成的攻击伤害+1。"], fate_bloodseeker_thirst:["嗜血","对1血角色使用攻击时，其须额外使用1张闪避。"], fate_bloodseeker_slaughter:["屠戮","杀死角色后可回复至上限。"],
  ...
  fate_nyx_burn:["法力燃烧","出牌阶段限一次：弃1张攻击，先弃目标1张手牌，再猜其另一张手牌颜色，猜对则弃置；最后可弃1张手牌，将目标因此弃掉的1张牌交给任意角色。"], fate_antimage_mana:["法力损毁","攻击造成伤害后可弃目标1张手牌。"], fate_antimage_blink:["闪烁","黑色手牌可当闪避使用或打出。"], fate_chen_judgement:["神判","判定生效后可弃1手牌令其重新判定。"], fate_chen_body:["特殊体质","血量不高于2时，攻击对你无效。"],
