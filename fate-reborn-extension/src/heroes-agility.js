...
export const AGILITY_HERO_SKILLS = {
  ...
  fate_bloodseeker_rupture: { trigger: { global: "fateCastPhase" }, forced: true, filter(event, player) { return player.countCards("h", card => get.color(card) === "red") > 0; }, async content(event, trigger, player) { if (player === game.me && !(await player.chooseBool("血之狂暴：是否发动？").forResult()).bool) return; const target = (await player.chooseTarget("血之狂暴：选择一名角色", true).set("ai", target => hostileTarget(target, player)).forResult()).targets?.[0]; if (!target) return; const paid = await player.chooseToDiscard("h", 1, true).set("filterCard", card => get.color(card) === "red").forResult(); if (paid.bool) await player.useCard({ name: "fate_bloodseeker_rupture_card", isCard: true }, target); } },
  fate_bloodseeker_rupture_effect: { charlotte: true, mod: { cardEnabled(card) { if (get.type(card) !== "basic") return false; }, }, trigger: { source: "damageBegin1" }, forced: true, filter(event) { return event.card?.name === "sha"; }, async content(event, trigger) { trigger.num += 1; } },
...
export const AGILITY_HERO_TRANSLATIONS = Object.fromEntries(Object.entries({
  ...
  fate_bloodseeker_rupture:["血之狂暴","施法阶段可弃红色手牌，令目标本回合只能用基本牌，攻击伤害+1。"], fate_bloodseeker_thirst:["嗜血","对1血角色使用攻击时，其须额外使用1张闪避。"], fate_bloodseeker_slaughter:["屠戮","杀死角色后可回复至上限。"],
