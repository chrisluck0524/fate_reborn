...
export const INTELLIGENCE_HERO_SKILLS = {
  ...
  fate_necro_heartstopper: { trigger: { global: "fateCastPhase" }, forced: true, filter(event, player) { return player.countCards("h", card => get.suit(card) === "spade") > 0 && game.hasPlayer(target => target !== player); }, async content(event, trigger, player) { const useful = target => target !== player && isUsefulCastTarget("heartstopper", { attitude: get.attitude(player, target), damaged: target.isDamaged() }); if (player !== game.me && !game.hasPlayer(useful)) return; if (player === game.me && !(await player.chooseBool("竭心光环：是否发动？").forResult()).bool) return; const target = (await player.chooseTarget("竭心光环：选择一名角色", true, other).set("ai", target => useful(target) ? hostileTarget(target, player) : -Infinity).forResult()).targets?.[0]; if (!target) return; const paid = await discard(player, 1, "竭心光环：将1张黑桃手牌置入状态区", card => get.suit(card) === "spade"); if (paid.bool) await player.useCard({ name: "fate_heartstopper_effect_card", isCard: true }, target); } },
  ...
  fate_crystal_frost: { trigger: { global: "fateCastPhase" }, forced: true, filter(event, player) { return player.countCards("h") >= 2 && game.hasPlayer(target => target !== player); }, async content(event, trigger, player) { const useful = target => target !== player && isUsefulCastTarget("crystal_frost", { attitude: get.attitude(player, target), hand: target.countCards("h") }); if (player !== game.me && (player.countCards("h") < 3 || !game.hasPlayer(useful))) return; if (player === game.me && !(await player.chooseBool("冰霜禁制：是否发动？").forResult()).bool) return; const target = (await player.chooseTarget("冰霜禁制：选择一名角色", true, other).set("ai", target => useful(target) ? hostileTarget(target, player) : -Infinity).forResult()).targets?.[0]; if (!target) return; const paid = await discard(player, 2, "冰霜禁制：弃置2张手牌"); if (paid.bool) await player.useCard({ name: "fate_crystal_frost_effect_card", isCard: true }, target); } },
  fate_crystal_frost_effect: { charlotte: true, trigger: { global: "useCardToTargeted" }, forced: true, popup: false, filter(event, player) { return event.card?.name === "sha" && event.player !== player && event.target === player; }, async content(event, trigger, player) { const map = trigger.customArgs || (trigger.customArgs = { default: {} }); map[player.playerid] ||= {}; const current = map[player.playerid].shanRequired || 1; map[player.playerid].shanRequired = requiredDodges({ base: current, frozen: true }); } },
  ...
  fate_enchantress_control: { trigger: { player: "useCardAfter" }, direct: true, filter(event) { const magic = event.respondTo?.[1]; let current = event; for (let depth = 0; current && depth < 8; depth += 1) { const original = current.name === "useCard" && current.card === magic ? current : current._trigger?.name === "useCard" && current._trigger.card === magic ? current._trigger : null; if (original) return event.card?.name === "wuxie" && magic?.name?.startsWith("fate_") && original._neutralized === true; current = current.parent; } return false; }, async content(event, trigger, player) { const magic = trigger.respondTo?.[1]; if (magic && (await player.chooseBool("魔法掌控：收回被成功驱散的魔法牌？").set("ai", () => true).forResult()).bool) await player.gain(magic, "gain2"); } },
};
