...
export const INTELLIGENCE_HERO_SKILLS = {
  ...
  fate_ice_prison_release: { charlotte: true, trigger: { global: "phaseAfter" }, forced: true, popup: false, filter(event, player) { return event.player === player; }, async content(event, trigger, player) { await releaseIcePrison(player, game.players.concat(game.dead)); } },
