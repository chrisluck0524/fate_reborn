...
async function dazzleWave(player) {
  ...
  const suitResult = await player.chooseControl("黑桃", "红桃", "梅花", "方块").set("prompt", "暗影波：选择弃置的花色").forResult();
  ...
  const hurt = await player.chooseTarget("暗影波：选择另一名角色受到1点伤害", true, other).forResult();
...
async function keeperWave(player) {
  ...
  for (const target of targets.targets || []) await player.useCard({ name: "fate_keeper_wave_effect", isCard: true }, target);
}
async function techiesBomb(player) {
  ...
  if (result.cards?.length) player.addToExpansion(result.cards, "give").gaintag.add("fate_bomb");
}
...
const active = {
  ...
  fate_dazzle_wave: { info: { usable: 1, filter: (event, player) => player.countCards("h") >= 2 }, run: dazzleWave },
...
export const INTELLIGENCE_HERO_CARDS = {
  fate_jakiro_ice_effect: trick(other, async (event, trigger, player) => { const target = event.target; const cards = target.getCards("h").randomGets(2); if (cards.length) { await target.lose(cards, ui.special); const expansion = await player.addToExpansion(cards, "gain2"); expansion.gaintag.add("fate_ice_prison"); } player.storage.fate_ice_prison_target = target.playerid; player.addTempSkill("fate_ice_prison_release", "phaseAfter"); }),
  ...
  fate_crystal_frost_effect_card: trick(other, async event => { event.target.addTempSkill("fate_crystal_frost_effect", { global: "phaseAfter" }); }),
...
export const INTELLIGENCE_HERO_SKILLS = {
  ...
  fate_necro_heartstopper: { trigger: { global: "fateCastPhase" }, forced: true, filter(event, player) { return player.countCards("h", card => get.suit(card) === "spade") > 0 && game.hasPlayer(target => target !== player); }, async content(event, trigger, player) { if (player === game.me && !(await player.chooseBool("竭心光环：是否发动？").forResult()).bool) return; const target = (await player.chooseTarget("竭心光环：选择一名角色", true, other).set("ai", target => hostileTarget(target, player)).forResult()).targets?.[0]; if (!target) return; const paid = await discard(player, 1, "竭心光环：将1张黑桃手牌置入状态区", card => get.suit(card) === "spade"); if (paid.bool) await player.useCard({ name: "fate_heartstopper_effect_card", isCard: true }, target); } },
  fate_heartstopper_effect: { charlotte: true, trigger: { player: "recoverBefore" }, forced: true, async content(event, trigger) { trigger.cancel(); } },
  ...
  fate_ice_prison_release: { charlotte: true, trigger: { player: "phaseAfter" }, forced: true, popup: false, async content(event, trigger, player) { const target = game.players.concat(game.dead).find(current => current.playerid === player.storage.fate_ice_prison_target); const cards = player.getExpansions("fate_ice_prison"); if (cards.length && target?.isAlive()) await target.gain(cards, "gain2"); else if (cards.length) await player.loseToDiscardpile(cards); delete player.storage.fate_ice_prison_target; } },
  fate_jakiro_liquid_fire: { enable: "chooseToUse", filterCard: true, selectCard: 2, position: "h", viewAs: { name: "sha", nature: "fate_fire" }, filterCard(card) { return get.color(card) === "red"; }, prompt: "将2张红色手牌当火焰攻击使用", trigger: { source: "damageBegin1" }, forced: true, filter(event) { return game.hasNature(event, "fate_fire"); }, async content(event, trigger) { trigger.num += 1; } },
  fate_crystal_frost: { trigger: { global: "fateCastPhase" }, forced: true, filter(event, player) { return player.countCards("h") >= 2 && game.hasPlayer(target => target !== player); }, async content(event, trigger, player) { if (player === game.me && !(await player.chooseBool("冰霜禁制：是否发动？").forResult()).bool) return; const target = (await player.chooseTarget("冰霜禁制：选择一名角色", true, other).set("ai", target => hostileTarget(target, player)).forResult()).targets?.[0]; if (!target) return; const paid = await discard(player, 2, "冰霜禁制：弃置2张手牌"); if (paid.bool) await player.useCard({ name: "fate_crystal_frost_effect_card", isCard: true }, target); } },
  fate_crystal_frost_effect: { charlotte: true, trigger: { global: ["useCardToPlayered", "useCardToTargeted"] }, forced: true, popup: false, filter(event, player) { return event.card?.name === "sha" && (event.player === player || event.target === player); }, async content(event, trigger, player) { const target = trigger.player === player ? trigger.target : player; const map = trigger.getParent().customArgs; map[target.playerid] ||= {}; const current = map[target.playerid].shanRequired || 1; map[target.playerid].shanRequired = current * 2; } },
  ...
  fate_dazzle_grave: { trigger: { global: "fateCastPhase" }, forced: true, filter(event, player) { return player.countCards("h", card => get.color(card) === "red") > 0; }, async content(event, trigger, player) { if (player === game.me && !(await player.chooseBool("薄葬：是否发动？").forResult()).bool) return; const target = (await player.chooseTarget("薄葬：选择一名角色", true).set("ai", target => get.attitude(player, target) * (target.hp <= 1 ? 4 : 1)).forResult()).targets?.[0]; if (!target) return; const paid = await discard(player, 1, "薄葬：弃置1张红色手牌", card => get.color(card) === "red"); if (paid.bool) await player.useCard({ name: "fate_shallow_grave_effect_card", isCard: true }, target); } },
  fate_shallow_grave: { charlotte: true, trigger: { player: "dying" }, forced: true, async content(event, trigger, player) { if (player.hp <= 0) await player.recover(1 - player.hp); } },
  ...
  fate_ogre_multicast: { trigger: { player: "useCardAfter" }, direct: true, filter(event) { return MAGIC_NAMES.has(event.card?.name) && !event.card.storage?.fateMulticast; }, async content(event, trigger, player) { const judge = await player.judge(card => get.color(card) === "red" ? 1 : -1).forResult(); if (!judge.bool) return; const target = (await player.chooseTarget(`多重施法：选择再次结算【${get.translation(trigger.card)}】的目标`, true, (card, source, current) => current.isAlive()).set("ai", target => hostileTarget(target, player)).forResult()).targets?.[0]; if (target) await player.useCard({ name: trigger.card.name, isCard: true, storage: { fateMulticast: true } }, target); } },
  ...
  fate_keeper_chakra: { enable: "chooseToUse", usable: 1, filterCard(card) { return get.color(card) === "black"; }, position: "h", viewAs: { name: "fate_chakra" }, prompt: "将1张黑色手牌当【查克拉】使用" },
  fate_keeper_favor: { enable: "phaseUse", async content(event, trigger, player) { await keeperFavor(player); } },
  ...
  fate_techies_detonate: { trigger: { global: ["useCardBefore", "respondBefore"] }, direct: true, filter(event, player) { return event.player !== player && player.getExpansions("fate_bomb").some(card => get.suit(card) === get.suit(event.card)); }, async content(event, trigger, player) { const bombs = player.getExpansions("fate_bomb").filter(card => get.suit(card) === get.suit(trigger.card)); if (!(await player.chooseBool(`引爆：弃置${bombs.length}枚同花色炸弹，摸牌并造成伤害？`).set("ai", () => true).forResult()).bool) return; await player.loseToDiscardpile(bombs); await player.draw(bombs.length); await trigger.player.damage({ num: bombs.length, source: player }); } },
  fate_techies_suicide: { trigger: { player: "die" }, direct: true, filter(event) { return event.source?.isAlive(); }, async content(event, trigger, player) { const judge = await trigger.source.judge(card => get.color(card) === "red" ? 1 : -1).forResult(); if (judge.bool) await trigger.source.damage({ num: 2, source: player }); } },
  fate_enchantress_heal: { trigger: { player: "recoverEnd" }, direct: true, filter(event, player) { return event.num > 0 && player.countCards("h", card => get.color(card) === "red") > 0; }, async content(event, trigger, player) { if (!(await player.chooseBool("治疗术：弃1张红色手牌，令另一名角色回复1点血？").forResult()).bool) return; const target = (await player.chooseTarget("治疗术：选择另一名受伤角色", true, (card, source, target) => source !== target && target.isDamaged()).forResult()).targets?.[0]; if (!target) return; const paid = await discard(player, 1, "治疗术：弃置1张红色手牌", card => get.color(card) === "red"); if (paid.bool) await target.recover(); } },
  fate_enchantress_dispel: { enable: "chooseToUse", filterCard(card) { return get.suit(card) === "heart"; }, position: "h", viewAs: { name: "wuxie" }, prompt: "将红桃手牌当【驱散】打出" },
  fate_enchantress_control: { trigger: { player: "useCardAfter" }, direct: true, filter(event) { return event.card?.name === "wuxie" && event.respondTo?.[1]?.name?.startsWith("fate_"); }, async content(event, trigger, player) { const magic = trigger.respondTo?.[1]; if (magic && (await player.chooseBool("魔法掌控：收回被成功驱散的魔法牌？").set("ai", () => true).forResult()).bool) await player.gain(magic, "gain2"); } },
};
...
export const INTELLIGENCE_HERO_TRANSLATIONS = Object.fromEntries(Object.entries({
  ...
  fate_necro_heartstopper: ["竭心光环", "施法阶段可弃1张黑桃手牌，令一名角色本回合无法回复血量。"], fate_necro_sadism: ["施虐之心", "其他角色死亡后可摸2张牌。"],
  fate_jakiro_ice: ["冰封", "出牌阶段限一次：弃1张牌，抽取其他角色2张手牌暗置于桌面；你的回合结束时，该角色收回这些牌。"], fate_jakiro_liquid_fire: ["液态火", "将2张红色手牌当火焰攻击使用；伤害+1。"],
  fate_crystal_frost: ["冰霜禁制", "施法阶段可弃2张手牌，令目标本回合使用攻击或闪避时须额外弃置同名牌。"], fate_crystal_aura: ["辉煌光环", "摸牌阶段可少摸1张，查看牌堆顶X张牌并分给X名不同角色（X不超过存活角色数）。"],
  fate_lich_feast: ["邪恶祭祀", "出牌阶段限一次：弃1张红色手牌，摸2张牌。"], fate_lich_armor: ["霜冻护甲", "受到伤害后可弃掉伤害来源1张牌。"],
  fate_dazzle_grave: ["薄葬", "施法阶段可弃1张红色手牌，令目标本回合濒死时回复至1点血。"], fate_dazzle_wave: ["暗影波", "出牌阶段限一次：弃2张同花色牌，令一名受伤角色回复1点血，并令另一名角色受到1点伤害。"],
  fate_ogre_multicast: ["多重施法", "魔法牌未被驱散且结算后可判定；红色时可更改目标，再结算一次该效果。"], fate_keeper_wave: ["冲击波", "出牌阶段限一次：弃3张不同花色牌，对至多2名角色各造成1点伤害。"], fate_keeper_chakra: ["查克拉", "每回合限一次：将1张黑色手牌当【查克拉】使用。"], fate_keeper_favor: ["恩惠", "你可以对任意1名角色使用【查克拉】。"],
  ...
  fate_enchantress_heal: ["治疗术", "回复血量后可弃1张红色手牌，令另一名受伤角色回复1点血。"], fate_enchantress_dispel: ["驱散精灵", "红桃手牌可当【驱散】使用。"], fate_enchantress_control: ["魔法掌控", "你可将你成功驱散的魔法牌收为手牌。"],
